...
async function init() {
    ...
    model = await tf.loadLayersModel('http://localhost:5500/start/tfjs_model/model.json');
    console.log(model);
    isModelLoaded = true;

    //Memanggil word_index
    // const word_indexjson = await fetch('http://127.0.0.1:5500/word_index.json'); // Untuk VS Code Live Server
    const word_indexjson = await fetch('http://localhost:5500/start/word_index.json');
